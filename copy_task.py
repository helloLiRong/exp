...
class CopyTask(object):
    ...
    def _create_datas(self, batch_size, min_step, max_step):
        ...
        for _ in range(batch_size):
            ...
            valid_data_x = np.concatenate([[self.get_start_symbol()], valid_data_x, [self.get_end_symbol()]], 0)
            res_x.append(np.concatenate(
                [valid_data_x, [self.blank_symbol for _ in range(step + 2, self.real_max_step)]], 0)
            )
            ...
            valid_data_y = np.concatenate([output_blanks[:step + 2, :], valid_data_y, output_blanks[step + 2:, :]], 0)
    ...
    def get_model(self, hidden_units, access_config, lr):
    ...
    def show_result(self, predicts, inputs, labels, show_all=True):
        ...
        for i in range(len(predicts)):
            predicts_strs = ''.join(
                [str(x).rjust(3, " ") for x in np.argmax(predicts[i], 1) if x != self.output_size - 1])
            labels_strs = ''.join([str(x).rjust(3, " ") for x in np.argmax(labels[i], 1) if x != self.output_size - 1])
